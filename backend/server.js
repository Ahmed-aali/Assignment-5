...
const db = new Pool({
  user: 'postgres.ecjpgkuiajydoqnvccmp',
  password: 'YourNewPasswordHere', // Put the new password here
...
app.delete('/student/:id', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < result.rows.length; i++) {
      const newId = i + 1;
      await db.query('UPDATE student SET id = $1 WHERE id = $2', [
        newId,
        result.rows[i].id,
      ]);
    }
...
app.delete('/teacher/:id', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < result.rows.length; i++) {
      const newId = i + 1;
      await db.query('UPDATE teacher SET id = $1 WHERE id = $2', [
        newId,
        result.rows[i].id,
      ]);
    }
